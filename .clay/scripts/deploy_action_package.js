...
async function deployActionPackage(){
  try {
    ...
    const actionPackageTmpDir = path.resolve('./.clay/action_package_zip_files/')
    if(!fs.existsSync(actionPackageTmpDir) ){
      console.log('Creating directory for zipping action package:', actionPackageTmpDir)
    ...
    output.on('close', async function() {
      ...
      try {
        ...
        if(err && err.response && err.response.body){
          console.log('Action package error response body: ', err.response.body)
        }
        else if(err && err.response){
          console.log('Action package error response: ', err.response)
        }
        else {
